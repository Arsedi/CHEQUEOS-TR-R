...
function parsePdfText(fullText, fileName) {
  ...
  for (const code of CODIGOS_CHEQUEO) {
    const re = new RegExp(code + "\\s+\\d{3,4}", "g");
  ...
  // Count unique days: pdf.js puts date and first code of that day on same visual row.
  // Each date DD/MM/YYYY appears once per day. We check within 60 chars for a chequeo code.
  // This works because pdf.js reads left-to-right: "01/07/2026 07:15 CRI 8484..."
  ...
  while ((dm3 = dateTokenRe.exec(fullText)) !== null) {
    ...
    // Check 60 chars after date for a chequeo code
    const chunk = fullText.substring(dm3.index, dm3.index + 60);
