          ...
          <div style={{marginTop:20}}>
            ...
            {!pdfParsed && (
              <>
                ...
                <div style={{display:"flex",alignItems:"center",gap:10,margin:"20px 0 16px",color:"#7a8fa6",fontSize:13}}>
                  <div style={{flex:1,height:1,background:"#2d3f52"}}/> ajustes al garantizado <div style={{flex:1,height:1,background:"#2d3f52"}}/>
                </div>
                <button onClick={()=>setReduccionOpen(o=>!o)} style={{width:"100%",background:"#1a2533",border:"1px solid #2d3f52",color:"#e8edf2",padding:"13px 14px",borderRadius:12,fontSize:13,fontWeight:600,cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",textAlign:"left",marginBottom:reduccionOpen?12:0}}>
                  <span>⚠️ Reducción al garantizado <span style={{color:"#7a8fa6",fontWeight:400,fontSize:11.5}}>(incapacidad, licencia, cambio voluntario)</span></span>
                  <span style={{display:"inline-block",transition:"transform 0.2s",transform:reduccionOpen?"rotate(180deg)":"rotate(0deg)"}}>▼</span>
                </button>
      ...
      <div style={{position:"fixed",bottom:0,left:0,right:0,background:"#111d2b",borderTop:"1px solid #1e2d3d",display:"flex",justifyContent:"space-around",padding:"10px 0 calc(16px + env(safe-area-inset-bottom, 0px))",zIndex:50,transform:"translateZ(0)",WebkitTransform:"translateZ(0)"}}>
        {[{id:"resumen",label:"Resumen",icon:"📊"},{id:"planear",label:"Programado",icon:"📋"},{id:"volar",label:"Ejecutado",icon:"✈️"},{id:"valores",label:"Valores",icon:"💰"},...(isAdmin?[{id:"comparativo",label:"Admin",icon:"👑"}]:[])].map(tab=>(
          <button key={tab.id} onClick={()=>{
            ...
            setView(tab.id); setEditIdx(null); setEditForm(null); setPdfParsed(null); setConfirmarReemplazo(false); setReduccionOpen(false);
