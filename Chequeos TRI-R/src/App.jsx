...
import { initializeFirestore, persistentLocalCache, doc, getDoc, setDoc, onSnapshot, collection, getDocs } from "firebase/firestore";

// ===== FIREBASE =====
const firebaseConfig = {
  apiKey: "AIzaSyBy4iBf0DwmDr_5fgIurCCrngCq24b0hgk",
  authDomain: "chequeos-tri-r.firebaseapp.com",
  projectId: "chequeos-tri-r",
  storageBucket: "chequeos-tri-r.firebasestorage.app",
  messagingSenderId: "651064900820",
  appId: "1:651064900820:web:46275d7316e442d50b4f41",
  measurementId: "G-GQW35WS0YE"
};
const fbApp = initializeApp(firebaseConfig);
const auth = getAuth(fbApp);
const db = initializeFirestore(fbApp, { localCache: persistentLocalCache() }); // offline automático
const EMAIL_DOMINIO = "@chequeostrir.app";
const emailDeUsuario = u => `${u.trim().toLowerCase()}${EMAIL_DOMINIO}`;
const passwordDePin = pin => `TRIR-${pin}`; // Firebase exige mínimo 6 caracteres
const RESET_PIN_URL = "https://resetpin-651064900820.us-central1.run.app";
const ADMIN_USERNAME = "arsedi"; // único usuario que ve el Comparativo
...
const CATS = [
  ...
  { id:"rnpsim",  nombre:"RNP/RQA/SIM", unidad:"sesion", etiqueta:"por sesión",  color:"#1abc9c", codigos:"SIM · UPRT · RQA · RNP · LOFT" },
...
const PREFIJOS = { ed:["CHK"], profrec:["PROF","REC"], rnpsim:["SIM","UPRT","RQA","RNP","LOFT"], ftd:["FTD"] };
const TODOS_CODIGOS = [...TRIR_CODES,"ED","CHK","PROF","REC","SIM","UPRT","RQA","RNP","LOFT","FTD","CRM"];
...
// ===== MIGRACIÓN v1 -> v2 -> v4 (datos viejos intactos en números) =====
function migrarRegistro(r, valorTrirGlobal){
  if (r && r.prog) {
    return {...r,
      ...
      reduccion:{...emptyCounts(),...(r.reduccion||{})},
      reduccionLog: Array.isArray(r.reduccionLog)?r.reduccionLog:[] };
  }
  ...
  return { ano:r.ano, mes:r.mes, diasProgramados:r.diasProgramados||0, prog, ejec, valores,
    ...
    reduccion: emptyCounts(), reduccionLog: [] };
}
...
// (v4.0) Programado ajustado por reducción al garantizado (incapacidad, licencia, cambio voluntario)
function progAjustado(r, c){
  const p = (r.prog && r.prog[c]) || 0;
  const red = (r.reduccion && r.reduccion[c]) || 0;
  return Math.max(0, p - red);
}
function progAjustadoObj(r){
  const o = {};
  CAT_IDS.forEach(c=>{ o[c] = progAjustado(r,c); });
  return o;
}
function ingresoRegistro(r, valoresGlobal){
  return CAT_IDS.reduce((s,c)=>{
    const p=progAjustado(r,c), e=r.ejec[c]||0;
    if(p===0 && e===0) return s;
...
function ingresoSolo(r, valoresGlobal, campo){ // campo: "prog" | "ejec"
  return CAT_IDS.reduce((s,c)=>{
    const n = campo==="prog" ? progAjustado(r,c) : (r[campo][c]||0);
    if(n===0) return s;
...
function desglose(r, valoresGlobal){
  return CATS.map(c=>{
    const pOrig=r.prog[c.id]||0, red=(r.reduccion&&r.reduccion[c.id])||0, p=Math.max(0,pOrig-red), e=r.ejec[c.id]||0;
    if(p===0 && e===0 && pOrig===0) return null;
    const pago=Math.max(p,e);
    return { id:c.id, nombre:c.nombre, color:c.color, unidad:c.unidad, prog:p, progOrig:pOrig, reduccion:red, ejec:e, pago, monto:pago*valorCat(r,valoresGlobal,c.id) };
  }).filter(Boolean);
}

// ===== PARSER DE PDF v2 =====
function contarVuelo(t, code){
  const re = new RegExp("\\b"+code+"\\s+(?:[a-zA-Z0-9]+\\s+)?\\d{2,4}","gi");
...
function contarPrefijo(t, pref){
  const re = new RegExp("\\b"+pref+"[A-Z]*\\b","gi");
  return (t.match(re)||[]).length;
}
function parsePdfText(fullText, fileName){
  ...
  const codeNearRe = new RegExp("\\b("+TODOS_CODIGOS.join("|")+")","i");
...
export default function App(){
  ...
  const [reduccionOpen, setReduccionOpen] = useState(false);
  const [reduccionNuevo, setReduccionNuevo] = useState(()=>{ try{ return !localStorage.getItem("chequeos_v4_reduccion_visto"); } catch { return true; } });
  const [manualOpen, setManualOpen] = useState(false);
  const [reduccionForm, setReduccionForm] = useState(()=>({ ano:yearNow, mes:monthNow, motivo:"", activos:{}, cantidades:{} }));
  const [volarForm, setVolarForm] = useState(()=>({ ano:yearNow, mes:monthNow, dia:new Date().getDate(), cat:"trir", cantidad:"" }));
  ...
  const [updateModalOpen, setUpdateModalOpen] = useState(()=>{
    try { return !localStorage.getItem("chequeos_v4_update_visto"); } catch { return true; }
  });
  const [pdfLoading, setPdfLoading] = useState(false);

  // ===== NUBE: estado de sesión y sincronización =====
  const [user, setUser] = useState(null);
  const [authChecking, setAuthChecking] = useState(true);
  const [authView, setAuthView] = useState("bienvenida"); // bienvenida | crear | login | recuperar
  const [authForm, setAuthForm] = useState({ usuario:"", pin1:"", pin2:"", secreta:"" });
  const [loginForm, setLoginForm] = useState({ usuario:"", pin:"" });
  const [recForm, setRecForm] = useState({ usuario:"", secreta:"", pinNuevo:"" });
  const [authError, setAuthError] = useState("");
  const [authBusy, setAuthBusy] = useState(false);
  const [migracionInfo, setMigracionInfo] = useState(null); // datos locales detectados al crear cuenta
  ...
  useEffect(()=>{
    if(!user){ return; }
    const ref = doc(db,"datos",user.uid);
    const unsub = onSnapshot(ref, { includeMetadataChanges:true }, snap=>{
      if(snap.exists()){
        const fresh = migrarData(snap.data());
        skipNextSaveRef.current = true; // este cambio vino de la nube, no hay que reescribirlo
        setData(fresh);
        dataLoadedRef.current = true;
      }
      setSyncStatus(snap.metadata.hasPendingWrites ? "syncing" : (navigator.onLine ? "synced" : "offline"));
    }, err=>{ console.error(err); });
    return ()=>{ unsub(); dataLoadedRef.current=false; };
  },[user]);

  useEffect(()=>{
    const onOnline=()=>setSyncStatus(s=>s==="syncing"?s:"synced");
  ...
  useEffect(()=>{
    ...
    if(skipNextSaveRef.current){ skipNextSaveRef.current = false; return; } // recién llegó de la nube, no reenviar
    setDoc(doc(db,"datos",user.uid), data).catch(err=>console.error(err));
  },[data,user]);

  async function handleCrearCuenta(){
    setAuthError("");
    const usuario = authForm.usuario.trim().toLowerCase();
    if(!usuario){ setAuthError("Ingresa un usuario"); return; }
    if(!/^[a-z0-9._-]{3,}$/.test(usuario)){ setAuthError("Usuario inválido (mínimo 3 letras/números)"); return; }
    if(!/^\d{4}$/.test(authForm.pin1)){ setAuthError("El PIN debe tener 4 dígitos"); return; }
    if(authForm.pin1!==authForm.pin2){ setAuthError("Los PIN no coinciden"); return; }
    if(!authForm.secreta.trim()){ setAuthError("Ingresa tu palabra secreta"); return; }
    setAuthBusy(true);
    try{
      const usuarioRef = doc(db,"usuarios",usuario);
      const existe = await getDoc(usuarioRef);
      if(existe.exists()){ setAuthError("Ese usuario ya existe, elige otro"); setAuthBusy(false); return; }
      const cred = await createUserWithEmailAndPassword(auth, emailDeUsuario(usuario), passwordDePin(authForm.pin1));
      const uid = cred.user.uid;
      const secretaHash = await sha256Hex(authForm.secreta);
      await setDoc(usuarioRef, { uid, secretaHash, creado: Date.now() });
      const local = loadData();
      if(local.registros && local.registros.length>0){
        setMigracionInfo({ data: local, uid });
      } else {
        await setDoc(doc(db,"datos",uid), { valores: defaultValores(), registros: [] });
      }
    } catch(e){
      setAuthError(e.code==="auth/email-already-in-use" ? "Ese usuario ya existe, elige otro" : "No se pudo crear la cuenta, intenta de nuevo");
    }
    setAuthBusy(false);
  }

  async function handleSubirDatosLocales(){
    if(!migracionInfo) return;
    setAuthBusy(true);
    try{ await setDoc(doc(db,"datos",migracionInfo.uid), migracionInfo.data); }
    catch(e){ showToast("No se pudo subir, intenta de nuevo","err"); }
    setMigracionInfo(null);
    setAuthBusy(false);
  }
  async function handleEmpezarDeCero(){
    if(!migracionInfo) return;
    setAuthBusy(true);
    try{ await setDoc(doc(db,"datos",migracionInfo.uid), { valores: defaultValores(), registros: [] }); }
    catch(e){ showToast("No se pudo continuar, intenta de nuevo","err"); }
    setMigracionInfo(null);
    setAuthBusy(false);
  }
  ...
  async function handleLogin(){
    ...
    if(!usuario || !/^\d{4}$/.test(loginForm.pin)){ setAuthError("Completa usuario y PIN"); return; }
    setAuthBusy(true);
    try{ await signInWithEmailAndPassword(auth, emailDeUsuario(usuario), passwordDePin(loginForm.pin)); }
    catch(e){ setAuthError("Usuario o PIN incorrecto"); }
    setAuthBusy(false);
  }

  function handleLogout(){
    signOut(auth);
    setAuthView("bienvenida");
    setLoginForm({ usuario:"", pin:"" });
  ...
  async function handleRecuperarPin(){
    setAuthError("");
    const usuario = recForm.usuario.trim().toLowerCase();
    if(!usuario || !recForm.secreta.trim()){ setAuthError("Completa usuario y palabra secreta"); return; }
    if(!/^\d{4}$/.test(recForm.pinNuevo)){ setAuthError("El PIN nuevo debe tener 4 dígitos"); return; }
    setAuthBusy(true);
    try{
      const secretaHash = await sha256Hex(recForm.secreta);
      const resp = await fetch(RESET_PIN_URL, {
        method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({ username: usuario, secretaHash, nuevoPin: recForm.pinNuevo })
      });
      const json = await resp.json().catch(()=>({}));
      if(!resp.ok){ setAuthError(json.error || "No se pudo recuperar el PIN"); setAuthBusy(false); return; }
  ...
  // ===== COMPARATIVO ADMIN (solo visible/accesible para ADMIN_USERNAME) =====
  const isAdmin = user?.email === emailDeUsuario(ADMIN_USERNAME);
  const [adminData, setAdminData] = useState(null); // [{username, valores, registros}]
  const [adminLoading, setAdminLoading] = useState(false);
  const [adminPeriodo, setAdminPeriodo] = useState("mes");
  const [adminAnoMes, setAdminAnoMes] = useState({ ano:yearNow, mes:monthNow });
  ...
  async function cargarDatosAdmin(){
    ...
    try{
      ...
      for (const uDoc of usuariosSnap.docs){
        const { uid } = uDoc.data();
        if(!uid) continue;
        const dSnap = await getDoc(doc(db,"datos",uid));
        const d = dSnap.exists() ? migrarData(dSnap.data()) : { valores:defaultValores(), registros:[] };
        lista.push({ username: uDoc.id, ...d });
      }
      setAdminData(lista);
    } catch(e){
      console.error(e);
      showToast("No se pudo cargar el comparativo","err");
    }
    setAdminLoading(false);
  }

  useEffect(()=>{
    if(view==="comparativo" && isAdmin && adminData===null){ cargarDatosAdmin(); }
  },[view,isAdmin]);
  ...
  function handlePlanearAgregar(){
    ...
    if(idx<0){
      registros.push({ ano, mes, diasProgramados:0, prog:emptyCounts(), ejec:emptyCounts(), log:[], valores:{...data.valores}, reduccion:emptyCounts(), reduccionLog:[] });
      registros.sort((a,b)=>a.ano!==b.ano?a.ano-b.ano:a.mes-b.mes);
  ...
  // ===== (v4.0) REDUCCIÓN AL GARANTIZADO — incapacidad, licencia, cambio voluntario =====
  function recomputarReduccion(reg){
    const r=emptyCounts();
    (reg.reduccionLog||[]).forEach(en=>{ if(r[en.cat]!==undefined) r[en.cat]+= (en.cantidad||0); });
    return r;
  }

  function handleAplicarReduccion(){
    const ano=parseInt(reduccionForm.ano), mes=parseInt(reduccionForm.mes);
    const activos = CAT_IDS.filter(c=>reduccionForm.activos[c] && parseInt(reduccionForm.cantidades[c])>0);
    if(activos.length===0){ showToast("Marca al menos un tipo con cantidad","err"); return; }
    const registros=[...data.registros];
    let idx=registros.findIndex(r=>r.ano===ano&&r.mes===mes);
    if(idx<0){
      registros.push({ ano, mes, diasProgramados:0, prog:emptyCounts(), ejec:emptyCounts(), log:[], valores:{...data.valores}, reduccion:emptyCounts(), reduccionLog:[] });
    ...
    reg.reduccionLog=[...(reg.reduccionLog||[])];
    activos.forEach(c=>{
      reg.reduccionLog.push({ cat:c, cantidad:parseInt(reduccionForm.cantidades[c]), motivo:reduccionForm.motivo.trim() });
    });
    reg.reduccion=recomputarReduccion(reg);
    registros[idx]=reg;
    setData(d=>({...d, registros}));
    setReduccionForm(f=>({...f, motivo:"", activos:{}, cantidades:{}}));
    showToast(`Reducción aplicada a ${MESES[mes]} ${ano} ✓`);
  }

  function handleEliminarReduccion(regIdx, logIdx){
    const registros=[...data.registros];
    const reg={...registros[regIdx]};
    reg.reduccionLog=(reg.reduccionLog||[]).filter((_,i)=>i!==logIdx);
    reg.reduccion=recomputarReduccion(reg);
    registros[regIdx]=reg;
    setData(d=>({...d, registros}));
    showToast("Reducción eliminada");
  }
  ...
  const pinInputStyle={...inputStyle,fontSize:26,fontWeight:700,textAlign:"center",letterSpacing:10};
  const authWrap={minHeight:"100vh",background:"#0f1923",color:"#e8edf2",fontFamily:"'Inter','Segoe UI',sans-serif",display:"flex",flexDirection:"column"};
  const authHeader={background:"linear-gradient(135deg,#1a0a0a 0%,#7a1510 45%,#c0392b 100%)",padding:"22px 20px 18px"};
  const authBody={flex:1,maxWidth:480,width:"100%",margin:"0 auto",padding:"30px 22px 40px",boxSizing:"border-box"};

  // ===== PANTALLA DE CARGA INICIAL =====
  if(authChecking){
    return (
      <div style={{...authWrap,alignItems:"center",justifyContent:"center"}}>
        <div style={{color:"#7a8fa6",fontSize:14}}>Cargando…</div>
      </div>
    );
  }

  // ===== MIGRACIÓN: datos locales detectados al crear cuenta =====
  if(migracionInfo){
    const totalLocal = migracionInfo.data.registros.length;
    const ingresoLocal = migracionInfo.data.registros.reduce((s,r)=>s+ingresoRegistro(r,migracionInfo.data.valores),0);
    return (
      <div style={authWrap}>
        <div style={authHeader}>
          <div style={{fontSize:24,fontWeight:900,color:"#fff"}}>avianca</div>
          <div style={{fontSize:9,color:"rgba(255,255,255,0.55)",letterSpacing:3,textTransform:"uppercase",marginTop:4}}>TRI-R · Airbus A320</div>
        </div>
        <div style={authBody}>
          <h1 style={{fontSize:21,fontWeight:800,color:"#fff",marginBottom:8}}>¡Cuenta creada! ✓</h1>
          <p style={{fontSize:13.5,color:"#7a8fa6",marginBottom:22,lineHeight:1.5}}>Antes de continuar, encontramos algo importante:</p>
          <div style={{background:"rgba(243,156,18,0.09)",border:"1px solid rgba(243,156,18,0.35)",borderRadius:16,padding:18,marginBottom:18}}>
            <div style={{fontSize:14,fontWeight:700,color:"#f39c12",marginBottom:6}}>📱 Hay datos guardados en este celular</div>
            <div style={{fontSize:13,color:"#c9d4de",lineHeight:1.55}}>Este dispositivo ya tiene un historial de chequeos guardado localmente, de antes de activar la nube.</div>
            <div style={{display:"flex",justifyContent:"space-between",background:"#0f1923",borderRadius:10,padding:"10px 14px",marginTop:12,fontSize:13}}>
              <span>Meses registrados</span><b style={{color:"#2ecc71"}}>{totalLocal}</b>
            </div>
            <div style={{display:"flex",justifyContent:"space-between",background:"#0f1923",borderRadius:10,padding:"10px 14px",marginTop:8,fontSize:13}}>
              <span>Total histórico</span><b style={{color:"#2ecc71"}}>{fmt(ingresoLocal)}</b>
            </div>
          </div>
          <p style={{fontSize:13.5,color:"#7a8fa6",marginBottom:18}}>¿Quieres subir esta información a tu cuenta nueva en la nube?</p>
          <button disabled={authBusy} onClick={handleSubirDatosLocales} style={btnPrimary}>{authBusy?"Subiendo…":"☁️ Sí, subir mis datos a la nube"}</button>
          <button disabled={authBusy} onClick={handleEmpezarDeCero} style={btnSecondary}>Empezar de cero (no recomendado)</button>
        </div>
      </div>
    );
  }

  // ===== PANTALLAS DE LOGIN / CREAR CUENTA / RECUPERAR =====
  if(!user){
    const ErrorMsg = () => authError ? <div style={{fontSize:12.5,color:"#e74c3c",marginTop:-8,marginBottom:16,display:"flex",alignItems:"center",gap:5}}>⚠️ {authError}</div> : null;

    if(authView==="bienvenida") return (
      <div style={authWrap}>
        <div style={authHeader}>
          <div style={{fontSize:24,fontWeight:900,color:"#fff"}}>avianca</div>
          <div style={{fontSize:9,color:"rgba(255,255,255,0.55)",letterSpacing:3,textTransform:"uppercase",marginTop:4}}>TRI-R · Airbus A320</div>
        </div>
        <div style={{...authBody,justifyContent:"center",display:"flex",flexDirection:"column",flex:1}}>
          <div style={{width:72,height:72,borderRadius:20,background:"rgba(52,152,219,0.12)",border:"1px solid rgba(52,152,219,0.3)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:32,margin:"0 auto 22px"}}>☁️</div>
          <h1 style={{fontSize:21,fontWeight:800,color:"#fff",textAlign:"center",marginBottom:8}}>Tus chequeos, en la nube</h1>
          <p style={{fontSize:13.5,color:"#7a8fa6",textAlign:"center",marginBottom:30,lineHeight:1.55}}>Se guarda en línea. No la pierdes al borrar caché y queda igual en todos tus dispositivos.</p>
          <button onClick={()=>{setAuthError("");setAuthView("crear");}} style={btnPrimary}>Crear mi cuenta</button>
          <button onClick={()=>{setAuthError("");setAuthView("login");}} style={btnSecondary}>Ya tengo cuenta</button>
        </div>
        <div style={{textAlign:"center",padding:"8px 20px 20px",fontSize:10,color:"rgba(255,255,255,0.2)"}}>Herramienta de uso personal · No afiliada a avianca</div>
      </div>
    );

    if(authView==="crear") return (
      <div style={authWrap}>
        <div style={authHeader}>
          <div style={{fontSize:24,fontWeight:900,color:"#fff"}}>avianca</div>
          <div style={{fontSize:9,color:"rgba(255,255,255,0.55)",letterSpacing:3,textTransform:"uppercase",marginTop:4}}>TRI-R · Airbus A320</div>
        ...
        <div style={authBody}>
          <h1 style={{fontSize:21,fontWeight:800,color:"#fff",marginBottom:8}}>Crea tu cuenta</h1>
          <p style={{fontSize:13.5,color:"#7a8fa6",marginBottom:24,lineHeight:1.55}}>Elige un usuario y un PIN de 4 dígitos. Los vas a necesitar para entrar desde cualquier celular.</p>
          <div style={{marginBottom:16}}>
            <label style={labelStyle}>Usuario</label>
            <input style={inputStyle} placeholder="ej: diegoserrano" value={authForm.usuario} onChange={e=>setAuthForm(f=>({...f,usuario:e.target.value}))}/>
          </div>
          <div style={{marginBottom:16}}>
            <label style={labelStyle}>PIN (4 dígitos)</label>
            <input style={pinInputStyle} maxLength={4} inputMode="numeric" placeholder="••••" value={authForm.pin1} onChange={e=>setAuthForm(f=>({...f,pin1:e.target.value.replace(/\D/g,"").slice(0,4)}))}/>
          </div>
          <div style={{marginBottom:16}}>
            <label style={labelStyle}>Confirma el PIN</label>
            <input style={pinInputStyle} maxLength={4} inputMode="numeric" placeholder="••••" value={authForm.pin2} onChange={e=>setAuthForm(f=>({...f,pin2:e.target.value.replace(/\D/g,"").slice(0,4)}))}/>
          </div>
          <div style={{marginBottom:8}}>
            <label style={labelStyle}>Palabra secreta</label>
            <input style={inputStyle} placeholder="ej: nombre de tu primer avión" value={authForm.secreta} onChange={e=>setAuthForm(f=>({...f,secreta:e.target.value}))}/>
            <div style={{fontSize:11.5,color:"#5b6b7f",marginTop:6}}>Si olvidas el PIN, con esto lo recuperas tú mismo</div>
          </div>
          <div style={{height:10}}/>
          <ErrorMsg/>
          <button disabled={authBusy} onClick={handleCrearCuenta} style={btnPrimary}>{authBusy?"Creando…":"Crear cuenta"}</button>
          <button disabled={authBusy} onClick={()=>{setAuthError("");setAuthView("bienvenida");}} style={{background:"none",border:"none",color:"#5b9bd5",fontSize:13.5,cursor:"pointer",width:"100%",padding:8,fontWeight:600}}>← Volver</button>
        </div>
      </div>
    );

    if(authView==="recuperar") return (
      <div style={authWrap}>
        <div style={authHeader}>
          <div style={{fontSize:24,fontWeight:900,color:"#fff"}}>avianca</div>
          <div style={{fontSize:9,color:"rgba(255,255,255,0.55)",letterSpacing:3,textTransform:"uppercase",marginTop:4}}>TRI-R · Airbus A320</div>
        ...
        <div style={authBody}>
          <h1 style={{fontSize:21,fontWeight:800,color:"#fff",marginBottom:8}}>Recupera tu PIN</h1>
          <p style={{fontSize:13.5,color:"#7a8fa6",marginBottom:24,lineHeight:1.55}}>Responde tu palabra secreta para poner un PIN nuevo.</p>
          <div style={{marginBottom:16}}>
            <label style={labelStyle}>Usuario</label>
            <input style={inputStyle} placeholder="ej: diegoserrano" value={recForm.usuario} onChange={e=>setRecForm(f=>({...f,usuario:e.target.value}))}/>
          </div>
          <div style={{marginBottom:16}}>
            <label style={labelStyle}>Palabra secreta</label>
            <input style={inputStyle} placeholder="La que pusiste al crear tu cuenta" value={recForm.secreta} onChange={e=>setRecForm(f=>({...f,secreta:e.target.value}))}/>
          </div>
          <div style={{marginBottom:8}}>
            <label style={labelStyle}>PIN nuevo</label>
            <input style={pinInputStyle} maxLength={4} inputMode="numeric" placeholder="••••" value={recForm.pinNuevo} onChange={e=>setRecForm(f=>({...f,pinNuevo:e.target.value.replace(/\D/g,"").slice(0,4)}))}/>
          </div>
          <div style={{height:10}}/>
          <ErrorMsg/>
          <button disabled={authBusy} onClick={handleRecuperarPin} style={btnPrimary}>{authBusy?"Guardando…":"Guardar PIN nuevo"}</button>
          <button disabled={authBusy} onClick={()=>{setAuthError("");setAuthView("login");}} style={{background:"none",border:"none",color:"#5b9bd5",fontSize:13.5,cursor:"pointer",width:"100%",padding:8,fontWeight:600}}>← Volver a iniciar sesión</button>
        </div>
      </div>
    );

    // authView==="login" (por defecto)
    return (
      <div style={authWrap}>
        <div style={authHeader}>
          <div style={{fontSize:24,fontWeight:900,color:"#fff"}}>avianca</div>
        ...
        <div style={authBody}>
          <h1 style={{fontSize:21,fontWeight:800,color:"#fff",marginBottom:8}}>Inicia sesión</h1>
          <p style={{fontSize:13.5,color:"#7a8fa6",marginBottom:24,lineHeight:1.55}}>Entra con tu usuario y PIN para ver tu información en este dispositivo.</p>
          <div style={{marginBottom:16}}>
            <label style={labelStyle}>Usuario</label>
            <input style={inputStyle} placeholder="ej: diegoserrano" value={loginForm.usuario} onChange={e=>setLoginForm(f=>({...f,usuario:e.target.value}))}/>
          </div>
          <div style={{marginBottom:8}}>
            <label style={labelStyle}>PIN</label>
            <input style={pinInputStyle} maxLength={4} inputMode="numeric" placeholder="••••" value={loginForm.pin} onChange={e=>setLoginForm(f=>({...f,pin:e.target.value.replace(/\D/g,"").slice(0,4)}))}/>
          </div>
          <div style={{height:10}}/>
          <ErrorMsg/>
          <button disabled={authBusy} onClick={handleLogin} style={btnPrimary}>{authBusy?"Entrando…":"Entrar"}</button>
          <button disabled={authBusy} onClick={()=>{setAuthError("");setAuthView("bienvenida");}} style={{background:"none",border:"none",color:"#5b9bd5",fontSize:13.5,cursor:"pointer",width:"100%",padding:8,fontWeight:600}}>← Volver</button>
          <button disabled={authBusy} onClick={()=>{setAuthError("");setAuthView("recuperar");}} style={{background:"none",border:"none",color:"#7a8fa6",fontSize:13,cursor:"pointer",width:"100%",padding:8}}>¿Olvidaste tu PIN?</button>
        </div>
      </div>
    );
  }
  ...
  const totalProgU = regs.reduce((s,r)=>s+totU(progAjustadoObj(r)),0);
  ...
  return (
    <div style={{minHeight:"100vh",background:"#0f1923",color:"#e8edf2",fontFamily:"'Inter','Segoe UI',sans-serif",paddingBottom:90}}>
      ...
      {/* HEADER */}
      <div style={{position:"sticky",top:0,zIndex:10,transform:"translateZ(0)",WebkitTransform:"translateZ(0)"}}>
        <div style={{background:"linear-gradient(135deg,#1a0a0a 0%,#7a1510 45%,#c0392b 100%)",padding:"22px 20px 18px",position:"relative",overflow:"hidden"}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
            ...
            <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:6}}>
              <div style={{fontSize:10,color:"rgba(255,255,255,0.35)",letterSpacing:1,marginTop:4}}>v4.0</div>
              <div style={{display:"inline-flex",alignItems:"center",gap:6,background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.15)",borderRadius:20,padding:"5px 12px",fontSize:11,color:"rgba(255,255,255,0.75)",fontWeight:600}}>
                <span style={{width:7,height:7,borderRadius:"50%",background:syncStatus==="synced"?"#2ecc71":syncStatus==="syncing"?"#f39c12":"#7a8fa6"}}/>
              ...
              <>
                ...
                <div>
                  <button onClick={()=>{
                    ...
                    else { localStorage.setItem("chequeos_onboarding_done","1"); try{localStorage.setItem("chequeos_v4_update_visto","1");}catch{} setUpdateModalOpen(false); setOnboardingStep(-1); }
                  ...
                  {onboardingStep===0 && (
                    <button onClick={()=>{localStorage.setItem("chequeos_onboarding_done","1");try{localStorage.setItem("chequeos_v4_update_visto","1");}catch{} setUpdateModalOpen(false); setOnboardingStep(-1);}} style={{width:"100%",background:"transparent",border:"none",color:"#7a8fa6",padding:"10px",fontSize:14,cursor:"pointer"}}>Saltar tutorial</button>
                  )}
                </div>
              </>
            );
      ...
      {/* MODAL — ÚLTIMA ACTUALIZACIÓN (v4.0) */}
      {onboardingStep===-1 && updateModalOpen && (
        <div style={{position:"fixed",inset:0,background:"rgba(10,15,22,0.97)",zIndex:998,display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
          <div style={{background:"#1a2533",border:"1px solid #2d3f52",borderRadius:20,padding:"26px 22px",maxWidth:420,width:"100%",maxHeight:"85vh",overflowY:"auto"}}>
            <div style={{fontSize:11,color:"#5b9bd5",letterSpacing:2,textTransform:"uppercase",marginBottom:8}}>🆕 Última actualización · v4.0</div>
            <div style={{fontSize:21,fontWeight:800,color:"#fff",marginBottom:14,lineHeight:1.3}}>Reducción al garantizado</div>
            <div style={{fontSize:14,color:"#c9d4de",lineHeight:1.6,marginBottom:14}}>
              Si te incapacitas, sales a licencia, o hay un cambio voluntario en tu programación que te hace perder chequeos que ya tenías programados, ahora puedes registrarlo — para que la garantía no te cuente esos chequeos que no pudiste volar por esa razón.
            </div>
            <div style={{fontSize:13,color:"#7a8fa6",lineHeight:1.6,marginBottom:18}}>
              Lo encuentras en <b style={{color:"#e8edf2"}}>Programado → ⚠️ Reducción al garantizado</b>.
            </div>
            <div style={{fontSize:12,color:"#7a8fa6",lineHeight:1.6,background:"rgba(255,255,255,0.05)",borderRadius:12,padding:"12px 14px",marginBottom:20}}>
              ⚠️ Esta app es una herramienta de uso y control personal. No está afiliada ni asociada a avianca ni a ninguna aerolínea.
            </div>
            <button onClick={()=>{ try{localStorage.setItem("chequeos_v4_update_visto","1");}catch{} setUpdateModalOpen(false); }} style={btnPrimary}>Entendido</button>
          </div>
        </div>
      )}
      ...
      <div style={{maxWidth:520,margin:"0 auto",padding:"0 16px"}}>
        ...
        {view==="resumen" && (
          <div>
            {/* Mes actual */}
            {mesActual && (()=>{
              const ejecU=totU(mesActual.ejec), progU=totU(progAjustadoObj(mesActual));
                      ...
                      {dsg.map(d=>(
                        <div key={d.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"5px 0",borderBottom:"1px solid rgba(255,255,255,0.05)"}}>
                          ...
                          <div style={{textAlign:"right"}}>
                            ...
                            <div style={{fontSize:10,color:"#7a8fa6"}}>{d.ejec}/{d.prog} {d.unidad==="sector"?"sect.":"ses."}{d.reduccion>0?<span style={{color:"#e67e22"}}> (-{d.reduccion} inc.)</span>:null}</div>
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            {regs.length>0 && (
              <>
                <div style={{background:"#1a2533",borderRadius:14,padding:"16px 18px",marginTop:16}}>
                  ...
                  {(()=>{
                    const barras = MESES_CORTO.map((m,i)=>{
                      ...
                      if(!r) return null;
                      const p=totU(progAjustadoObj(r)), e=totU(r.ejec);
                    ...
                    const H = 150;
                    const sel = barras.find(b=>grafSel===`${grafAnio}-${b.i}`) || barras.find(b=>b.curso) || barras[barras.length-1];
                    return (
                      <>
                        ...
                        {sel && (()=>{
                          const e2=sel.e>sel.p?{c:"#2ecc71",t:"▲ Extra"}:(sel.e===sel.p?{c:"#3498db",t:"Justo"}:{c:"#f39c12",t:"▼ Garantía"});
                          return (
                            <div style={{background:"#0f1923",border:"1px solid #2d3f52",borderRadius:12,padding:"12px 14px",marginTop:14}}>
                              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                                <div style={{fontSize:11,color:"#5b9bd5",textTransform:"uppercase",letterSpacing:1}}>📅 {MESES[sel.i]} {grafAnio}</div>
                                {sel.curso && <span style={{fontSize:9.5,fontWeight:800,color:"#3498db",background:"#3498db22",border:"1px solid #3498db55",borderRadius:20,padding:"2px 9px",letterSpacing:0.5}}>EN CURSO</span>}
                              </div>
                ...
                {(()=>{
                  ...
                  const progAnio=rsAnio.reduce((s,r)=>s+totU(progAjustadoObj(r)),0);
                ...
                {(()=>{
                  ...
                  const prog=rs.reduce((s,r)=>s+totU(progAjustadoObj(r)),0);
                    ...
                    {expanded && (
                      <div style={{border:"1px solid #2d3f52",borderTop:"none",borderRadius:"0 0 14px 14px",overflow:"hidden"}}>
                        {regsAnio.map((r,i)=>{
                          ...
                          const eU=totU(r.ejec), pU=totU(progAjustadoObj(r));
                          const diff=eU-pU;
        ...
        {view==="planear" && (
          <div style={{marginTop:20}}>
            ...
            {!pdfParsed && (
              <>
                <div style={{display:"flex",alignItems:"center",gap:10,margin:"4px 0 16px",color:"#7a8fa6",fontSize:13}}>
                  <div style={{flex:1,height:1,background:"#2d3f52"}}/> o gestiona manualmente <div style={{flex:1,height:1,background:"#2d3f52"}}/>
                </div>

                {/* ===== INGRESO MANUAL — desplegable ===== */}
                <button onClick={()=>setManualOpen(o=>!o)} style={{width:"100%",background:"#1a2533",border:"1px solid #2d3f52",color:"#e8edf2",padding:"13px 14px",borderRadius:12,fontSize:13,fontWeight:600,cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",textAlign:"left",marginBottom:manualOpen?12:10}}>
                  <span>✏️ Ingresar programado manualmente</span>
                  <span style={{display:"inline-block",transition:"transform 0.2s",transform:manualOpen?"rotate(180deg)":"rotate(0deg)"}}>▼</span>
                </button>
                ...
                {/* ===== (v4.0) REDUCCIÓN AL GARANTIZADO — desplegable ===== */}
                <button onClick={()=>{ setReduccionOpen(o=>!o); if(reduccionNuevo){ try{localStorage.setItem("chequeos_v4_reduccion_visto","1");}catch{} setReduccionNuevo(false); } }} style={{width:"100%",background:"#1a2533",border:"1px solid #2d3f52",color:"#e8edf2",padding:"13px 14px",borderRadius:12,fontSize:13,fontWeight:600,cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",textAlign:"left",marginBottom:reduccionOpen?12:0}}>
                  <span>⚠️ Reducción al garantizado <span style={{color:"#7a8fa6",fontWeight:400,fontSize:11.5}}>(incapacidad, licencia, cambio voluntario)</span>{reduccionNuevo && <span style={{marginLeft:8,fontSize:9.5,fontWeight:800,color:"#fff",background:"#c0392b",borderRadius:20,padding:"2px 8px",letterSpacing:0.5,verticalAlign:"middle"}}>NUEVO</span>}</span>
                  <span style={{display:"inline-block",transition:"transform 0.2s",transform:reduccionOpen?"rotate(180deg)":"rotate(0deg)",flexShrink:0,marginLeft:8}}>▼</span>
                </button>

                {reduccionOpen && (()=>{
                  const regReduccion = regs.find(x=>x.ano===parseInt(reduccionForm.ano)&&x.mes===parseInt(reduccionForm.mes));
                  const logReduccion = regReduccion?.reduccionLog || [];
                  const realIdxReduccion = regReduccion ? regs.findIndex(x=>x.ano===regReduccion.ano&&x.mes===regReduccion.mes) : -1;
                  return (
                    <div style={{background:"rgba(230,126,34,0.08)",border:"1px solid rgba(230,126,34,0.35)",borderRadius:14,padding:16,marginBottom:20}}>
                      <div style={{fontSize:12,color:"#e0a56f",lineHeight:1.5,marginBottom:16}}>Selecciona el mes al que aplica, los tipos afectados y cuántos se pierden de cada uno.</div>

                      <label style={labelStyle}>¿A qué mes aplica?</label>
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
                        <select value={reduccionForm.mes} onChange={e=>setReduccionForm(f=>({...f,mes:parseInt(e.target.value)}))} style={inputStyle}>
                          {MESES.map((m,i)=><option key={i} value={i}>{m}</option>)}
                        </select>
                        <input type="number" value={reduccionForm.ano} onChange={e=>setReduccionForm(f=>({...f,ano:e.target.value}))} style={inputStyle}/>
                      </div>

                      <label style={labelStyle}>Motivo (opcional, para tu referencia)</label>
                      <input type="text" placeholder="ej: incapacidad 5-6 agosto" value={reduccionForm.motivo} onChange={e=>setReduccionForm(f=>({...f,motivo:e.target.value}))} style={{...inputStyle,fontSize:13,marginBottom:16}}/>

                      <label style={{...labelStyle,marginBottom:10}}>Tipos afectados</label>
                      {CATS.map(c=>{
                        const activo = !!reduccionForm.activos[c.id];
                        return (
                          <div key={c.id} style={{display:"flex",alignItems:"center",gap:10,background:"#1a2533",border:"1px solid #2d3f52",borderRadius:10,padding:"9px 12px",marginBottom:8}}>
                            <input type="checkbox" checked={activo} onChange={e=>setReduccionForm(f=>({...f,activos:{...f.activos,[c.id]:e.target.checked}}))} style={{width:20,height:20,accentColor:"#e67e22",flexShrink:0}}/>
                            <div style={{flex:1,fontSize:14,fontWeight:700,color:c.color}}>{c.nombre}</div>
                            <input type="number" placeholder="0" disabled={!activo} value={reduccionForm.cantidades[c.id]||""} onChange={e=>setReduccionForm(f=>({...f,cantidades:{...f.cantidades,[c.id]:e.target.value}}))} style={{width:70,textAlign:"center",padding:8,fontSize:15,fontWeight:700,background:activo?"#0f1923":"#0a1219",border:"1px solid #2d3f52",borderRadius:8,color:"#e8edf2",opacity:activo?1:0.35}}/>
                          </div>
                        );
                      })}

                      <button onClick={handleAplicarReduccion} style={{width:"100%",background:"#e67e22",border:"none",color:"#fff",padding:13,borderRadius:12,fontSize:15,fontWeight:700,cursor:"pointer",marginTop:4}}>Aplicar reducción</button>

                      {logReduccion.length>0 && (
                        <div style={{marginTop:20}}>
                          <div style={{fontSize:11,color:"#7a8fa6",textTransform:"uppercase",letterSpacing:1,marginBottom:8}}>Reducciones aplicadas — {MESES[parseInt(reduccionForm.mes)]} {reduccionForm.ano}</div>
                          {logReduccion.map((en,i)=>(
                            <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:"#1a2533",border:"1px solid #2d3f52",borderRadius:10,padding:"10px 12px",marginBottom:7}}>
                              <div style={{fontSize:13}}>
                                <div style={{fontWeight:700,color:catById(en.cat).color}}>{catById(en.cat).nombre} <span style={{color:"#e67e22"}}>-{en.cantidad}</span></div>
                                {en.motivo && <div style={{fontSize:11,color:"#7a8fa6",marginTop:2}}>{en.motivo}</div>}
                              </div>
                              <button onClick={()=>handleEliminarReduccion(realIdxReduccion,i)} style={{background:"none",border:"none",color:"#e74c3c",cursor:"pointer",fontSize:14}}>✕</button>
                            </div>
                          ))}
                        </div>
                      )}
        ...
        {view==="volar" && (
          <div style={{marginTop:20}}>
            ...
            {(()=>{
              const mesSel = regs.find(r=>r.ano===volarForm.ano&&r.mes===volarForm.mes);
              ...
              return (
                <div style={{background:"rgba(231,76,60,0.1)",border:"1px solid rgba(231,76,60,0.3)",borderRadius:12,padding:16,marginBottom:16,fontSize:13,color:"#e74c3c"}}>
                  {MESES[volarForm.mes]} {volarForm.ano} aún no tiene programación. Súbela en "Programado" para poder registrar.
                </div>
              );
            })()}
            {(()=>{
              const meses = regs.map(r=>({ano:r.ano,mes:r.mes}));
              if (!meses.some(x=>x.ano===yearNow&&x.mes===monthNow)) meses.push({ano:yearNow,mes:monthNow});
              meses.sort((a,b)=> a.ano!==b.ano ? b.ano-a.ano : b.mes-a.mes);
              return (
                <div style={{marginBottom:16}}><label style={labelStyle}>Fecha</label>
                  <select
                    value={`${volarForm.ano}-${volarForm.mes}-${volarForm.dia}`}
                    onChange={e=>{const [a,m,d]=e.target.value.split("-");setVolarForm(f=>({...f,ano:parseInt(a),mes:parseInt(m),dia:parseInt(d)}));}}
                    style={inputStyle}>
                    {meses.map(mm=>(
                      Array.from({length:diasDelMes(mm.ano,mm.mes)},(_,i)=>i+1).map(d=>(
                        <option key={`${mm.ano}-${mm.mes}-${d}`} value={`${mm.ano}-${mm.mes}-${d}`}>{String(d).padStart(2,"0")} de {MESES[mm.mes]} {mm.ano}</option>
                      ))
                    ))}
                  </select>
                </div>
        ...
        {view==="valores" && (
          <div style={{marginTop:20}}>
            ...
            <div style={{borderTop:"1px solid #2d3f52",paddingTop:20,marginTop:20}}>
              <div style={{fontSize:13,color:"#7a8fa6",marginBottom:12,textTransform:"uppercase",letterSpacing:1}}>Mi cuenta</div>
              <div style={{background:"#1a2533",border:"1px solid #2d3f52",borderRadius:12,padding:"12px 16px",marginBottom:12,fontSize:13,color:"#9db2c7"}}>
                Sesión activa: <b style={{color:"#e8edf2"}}>{user?.email?.split("@")[0]}</b>
              </div>
              <button onClick={handleLogout} style={{width:"100%",background:"transparent",border:"1px solid #2d3f52",color:"#9db2c7",padding:"12px",borderRadius:12,fontSize:14.5,cursor:"pointer",marginBottom:20}}>Cerrar sesión</button>
            </div>

            <div style={{borderTop:"1px solid #2d3f52",paddingTop:20,marginTop:0}}>
        ...
        {/* ===== COMPARATIVO (solo admin) ===== */}
        {view==="comparativo" && isAdmin && (()=>{
          if(adminLoading || adminData===null){
            return (
              <div style={{marginTop:40,textAlign:"center",color:"#7a8fa6"}}>
                <div style={{fontSize:32,marginBottom:10}}>👑</div>
                <div>Cargando comparativo…</div>
              </div>
            );
          }
          const periodos = (()=>{
            const set = new Set();
            adminData.forEach(u=> u.registros.forEach(r=> set.add(r.ano+"-"+r.mes)) );
            return [...set].map(k=>{ const [ano,mes]=k.split("-").map(Number); return {ano,mes}; })
              .sort((a,b)=> a.ano!==b.ano ? a.ano-b.ano : a.mes-b.mes);
          })();
          function filasPara(ano,mes,soloAno){
            return adminData.map(u=>{
              let regs = u.registros;
              if(ano!=null) regs = regs.filter(r=> r.ano===ano && (soloAno || r.mes===mes));
              if(regs.length===0) return null;
              const prog = regs.reduce((s,r)=>s+totU(progAjustadoObj(r)),0);
              const ejec = regs.reduce((s,r)=>s+totU(r.ejec),0);
              const ingreso = regs.reduce((s,r)=>s+ingresoRegistro(r,u.valores),0);
              return { username:u.username, prog, ejec, ingreso };
            }).filter(Boolean).sort((a,b)=> b.ejec-a.ejec);
          }
          const rows = adminPeriodo==="mes" ? filasPara(adminAnoMes.ano, adminAnoMes.mes, false)
                     : adminPeriodo==="anio" ? filasPara(yearNow, null, true)
                     : filasPara(null, null, false);
          const rowsMoney = [...rows].sort((a,b)=> b.ingreso-a.ingreso);
          const maxEjecTop = Math.max(...rows.map(r=>Math.max(r.prog,r.ejec)),1);
          const maxIngreso = Math.max(...rowsMoney.map(r=>r.ingreso),1);
          return (
            <div style={{marginTop:20}}>
              <div style={{fontSize:18,fontWeight:700,marginBottom:2,display:"flex",alignItems:"center",gap:8}}>👑 Comparativo</div>
              <div style={{fontSize:12.5,color:"#7a8fa6",marginBottom:18}}>Solo tú puedes ver esta pantalla — invisible para los demás.</div>

              <div style={{display:"flex",gap:6,marginBottom:14,overflowX:"auto"}}>
                {[["mes","Mes"],["anio",`Año ${yearNow}`],["global","Global (siempre)"]].map(([id,lbl])=>(
                  <button key={id} onClick={()=>setAdminPeriodo(id)} style={{background:adminPeriodo===id?"#c0392b":"rgba(255,255,255,0.06)",border:adminPeriodo===id?"none":"1px solid #2d3f52",color:adminPeriodo===id?"#fff":"#7a8fa6",padding:"6px 16px",borderRadius:20,fontSize:12.5,fontWeight:adminPeriodo===id?700:600,cursor:"pointer",whiteSpace:"nowrap"}}>{lbl}</button>
                ))}
              </div>

              {adminPeriodo==="mes" && (
                <div style={{background:"#1a2533",border:"1px solid #2d3f52",borderRadius:12,padding:"10px 14px",marginBottom:16}}>
                  <label style={{...labelStyle,marginBottom:8}}>Mes a comparar</label>
                  <select value={`${adminAnoMes.ano}-${adminAnoMes.mes}`} onChange={e=>{const [a,m]=e.target.value.split("-").map(Number); setAdminAnoMes({ano:a,mes:m});}} style={{...inputStyle, WebkitAppearance:"none"}}>
                    {periodos.slice().reverse().map(p=>(
                      <option key={`${p.ano}-${p.mes}`} value={`${p.ano}-${p.mes}`}>{MESES[p.mes]} {p.ano}</option>
                    ))}
                  </select>
                </div>
              )}
              ...
              {rows.length>0 && (
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
                  <div style={{background:"#1a2533",borderRadius:14,padding:"12px 14px"}}>
                    <div style={{fontSize:10,color:"#7a8fa6",textTransform:"uppercase",letterSpacing:1}}>Instructores activos</div>
                    <div style={{fontSize:20,fontWeight:800,color:"#3498db",marginTop:4}}>{rows.length}</div>
                  </div>
                  <div style={{background:"#1a2533",borderRadius:14,padding:"12px 14px"}}>
                    <div style={{fontSize:10,color:"#7a8fa6",textTransform:"uppercase",letterSpacing:1}}>Va más adelante</div>
                    <div style={{fontSize:16,fontWeight:800,color:"#2ecc71",marginTop:4,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{rows[0].username}{rows[0].username===ADMIN_USERNAME?" (tú)":""}</div>
                  </div>
                </div>
              )}

              {rows.length===0 ? (
                <div style={{background:"#1a2533",borderRadius:14,padding:26,textAlign:"center",color:"#7a8fa6",fontSize:13}}>Sin datos para este período.</div>
              ) : (
              <>
              <div style={{background:"#1a2533",borderRadius:16,padding:"16px 18px",marginBottom:16}}>
                <div style={{fontSize:12,color:"#7a8fa6",textTransform:"uppercase",letterSpacing:1.5,marginBottom:14}}>✈️ Ejecutado vs Programado</div>
                {rows.map((r,i)=>{
                  const color = r.ejec>=r.prog ? "#2ecc71" : (i===0 ? "#3498db" : "#f39c12");
                  const pct = Math.min(100,(r.ejec/maxEjecTop)*100), pctMeta = Math.min(100,(r.prog/maxEjecTop)*100);
                  return (
                    <div key={r.username} style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
                      <div style={{width:20,fontSize:12,fontWeight:800,color:i===0?"#f1c40f":"#5b6b7f",textAlign:"center"}}>{i===0?"🏆":i+1}</div>
                      <div style={{width:74,flexShrink:0}}>
                        <div style={{fontSize:12.5,fontWeight:700,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{r.username}</div>
                        {r.username===ADMIN_USERNAME && <div style={{fontSize:9,color:"#5b9bd5",fontWeight:600}}>Tú</div>}
                      </div>
                      <div style={{flex:1}}>
                        <div style={{background:"rgba(255,255,255,0.08)",borderRadius:6,height:6,overflow:"hidden"}}><div style={{width:`${pctMeta}%`,height:"100%",background:"rgba(255,255,255,0.35)",borderRadius:6}}/></div>
                        <div style={{background:"#0f1923",borderRadius:6,height:16,overflow:"hidden",marginTop:3}}>
                          <div style={{width:`${pct}%`,height:"100%",background:color,borderRadius:6,display:"flex",alignItems:"center",justifyContent:"flex-end",paddingRight:6}}>
                            <span style={{fontSize:9.5,fontWeight:700,color:"#fff"}}>{r.ejec}</span>
                          </div>
                        </div>
                        <div style={{fontSize:10,color:"#7a8fa6",marginTop:3}}>{r.ejec} / {r.prog} sectores</div>
                      </div>
                    </div>
                  );
                })}
              ...
              <div style={{background:"#1a2533",borderRadius:16,padding:"16px 18px",marginBottom:16}}>
                <div style={{fontSize:12,color:"#7a8fa6",textTransform:"uppercase",letterSpacing:1.5,marginBottom:14}}>💰 Ganancias</div>
                {rowsMoney.map((r,i)=>{
                  const color = i===0 ? "#2ecc71" : (r.username===ADMIN_USERNAME ? "#3498db" : "#5b6b7f");
                  const pct = Math.max(8,(r.ingreso/maxIngreso)*100);
                  return (
                    <div key={r.username} style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
                      <div style={{width:20,fontSize:12,fontWeight:800,color:i===0?"#f1c40f":"#5b6b7f",textAlign:"center"}}>{i===0?"🏆":i+1}</div>
                      <div style={{width:70,fontSize:11.5,fontWeight:700,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{r.username}</div>
                      <div style={{flex:1,background:"#0f1923",borderRadius:6,height:22,overflow:"hidden"}}>
                        <div style={{width:`${pct}%`,height:"100%",background:color,borderRadius:6}}/>
                      </div>
                      <div style={{width:90,textAlign:"right",fontSize:12.5,fontWeight:700,color,flexShrink:0}}>{fmt(r.ingreso)}</div>
                    </div>
                  );
                })}
              </div>
              ...
              <div style={{background:"#1a2533",borderRadius:16,padding:"16px 18px",marginBottom:16}}>
                <div style={{fontSize:12,color:"#7a8fa6",textTransform:"uppercase",letterSpacing:1.5,marginBottom:10}}>📅 Historial por mes — quién va más</div>
                {periodos.length===0 && <div style={{fontSize:13,color:"#7a8fa6"}}>Sin historial todavía.</div>}
                {periodos.slice().reverse().map(p=>{
                  const filas = filasPara(p.ano,p.mes,false);
                  if(filas.length===0) return null;
                  const lider = filas[0];
                  const key = `${p.ano}-${p.mes}`;
                  const open = !!histOpen[key];
                  const maxE = Math.max(...filas.map(f=>f.ejec),1);
                  return (
                    <div key={key} style={{border:"1px solid #2d3f52",borderRadius:12,marginBottom:8,overflow:"hidden"}}>
                      <button onClick={()=>setHistOpen(h=>({...h,[key]:!h[key]}))} style={{width:"100%",background:"#1a2533",border:"none",display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 14px",cursor:"pointer"}}>
                        <div style={{display:"flex",alignItems:"center",gap:10}}>
                          <div style={{fontSize:13.5,fontWeight:700,width:64,textAlign:"left",color:"#e8edf2"}}>{MESES_CORTO[p.mes]} {p.ano}</div>
                          <div style={{fontSize:12.5,fontWeight:600,color:"#2ecc71"}}>🏆 {lider.username}{lider.username===ADMIN_USERNAME?" (tú)":""}</div>
                        </div>
                        <div style={{display:"flex",alignItems:"center",gap:8}}>
                          <span style={{fontSize:11,color:"#7a8fa6"}}>{lider.ejec}/{lider.prog} sect.</span>
                          <span style={{fontSize:12,color:"#5b6b7f",display:"inline-block",transform:open?"rotate(180deg)":"rotate(0deg)",transition:"transform 0.2s"}}>▼</span>
                        </div>
                      </button>
                      {open && (
                        <div style={{background:"#141f2e",padding:"6px 14px 12px"}}>
                          {filas.map((f,i)=>{
                            const color = f.ejec>=f.prog ? "#2ecc71" : (i===0?"#3498db":"#f39c12");
                            const pct = Math.max(6,(f.ejec/maxE)*100);
                            return (
                              <div key={f.username} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 0",borderBottom:i<filas.length-1?"1px solid #1c2733":"none"}}>
                                <div style={{width:16,fontSize:11,fontWeight:800,color:"#5b6b7f",textAlign:"center"}}>{i===0?"🏆":i+1}</div>
                                <div style={{width:74,fontSize:11.5,fontWeight:600,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{f.username}{f.username===ADMIN_USERNAME?" (tú)":""}</div>
                                <div style={{flex:1,background:"#0f1923",borderRadius:5,height:9,overflow:"hidden"}}><div style={{width:`${pct}%`,height:"100%",background:color,borderRadius:5}}/></div>
                                <div style={{width:64,textAlign:"right",fontSize:11,fontWeight:700,color}}>{f.ejec}/{f.prog}</div>
                              </div>
                            );
                          })}
                        </div>
              ...
              <div style={{background:"rgba(255,215,0,0.06)",border:"1px solid rgba(255,215,0,0.25)",borderRadius:12,padding:"12px 14px",fontSize:11.5,color:"#d9c98a",lineHeight:1.5}}>
                👑 Esta vista y estos datos solo existen para tu cuenta. Los demás instructores nunca ven esta pestaña — la restricción está también en las reglas de la base de datos, no solo en la app.
              </div>
            </div>
          );
        })()}
      ...
      {/* BOTTOM NAV */}
      <div style={{position:"fixed",bottom:0,left:0,right:0,background:"#111d2b",borderTop:"1px solid #1e2d3d",display:"flex",justifyContent:"space-around",padding:"10px 0 calc(16px + env(safe-area-inset-bottom, 0px))",zIndex:50,transform:"translateZ(0)",WebkitTransform:"translateZ(0)"}}>
        {[{id:"resumen",label:"Resumen",icon:"📊"},{id:"planear",label:"Programado",icon:"📋"},{id:"volar",label:"Ejecutado",icon:"✈️"},{id:"valores",label:"Valores",icon:"💰"},...(isAdmin?[{id:"comparativo",label:"Admin",icon:"👑"}]:[])].map(tab=>(
          <button key={tab.id} onClick={()=>{
            if(tab.id==="volar"){
              setVolarForm(f=>({...f,ano:yearNow,mes:monthNow,dia:new Date().getDate()}));
            }
            if(tab.id==="valores"){
              const o={}; CAT_IDS.forEach(c=>{ o[c]=data.valores[c]?String(data.valores[c]):""; });
              setValForm(o);
            }
            setView(tab.id); setEditIdx(null); setEditForm(null); setPdfParsed(null); setConfirmarReemplazo(false); setReduccionOpen(false); setManualOpen(false);
          }} style={{background:"none",border:"none",color:view===tab.id?(tab.id==="comparativo"?"#ffd966":"#e74c3c"):"#7a8fa6",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:3,fontSize:11,fontWeight:view===tab.id?700:400}}>
