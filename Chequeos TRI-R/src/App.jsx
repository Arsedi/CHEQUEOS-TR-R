...
function parsePdfText(fullText, fileName) {
  ...
  // Count days with chequeo by finding dates that appear near chequeo codes in pdf.js text.
  // pdf.js reads rows left-to-right, so date and first code of that day are close together.
  // We search within 80 chars of each date occurrence for a chequeo code.
  const codeNearRe = new RegExp("(" + CODIGOS_CHEQUEO.join("|") + ")\\s+\\d{3,4}");
  const dateTokenRe = /\d{2}\/\d{2}\/\d{4}/g;
  const daysSet = new Set();
  let dm3;
  while ((dm3 = dateTokenRe.exec(fullText)) !== null) {
    const chunk = fullText.substring(dm3.index, dm3.index + 80);
    if (codeNearRe.test(chunk)) daysSet.add(dm3[0]);
  }
  // Fallback: if window too small, try wider search grouped by date blocks
  let diasConChequeo = daysSet.size;
  if (diasConChequeo === 0 && totalSectores > 0) {
    // Split by dates and check each segment
    const segments = fullText.split(/(\d{2}\/\d{2}\/\d{4})/);
    for (let i = 1; i < segments.length - 1; i += 2) {
      const nearby = (segments[i+1] || "").substring(0, 200);
      if (codeNearRe.test(nearby)) daysSet.add(segments[i]);
    }
    diasConChequeo = daysSet.size;
  }
  // Last fallback
