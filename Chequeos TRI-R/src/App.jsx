...
const CODIGOS_CHEQUEO = ["CRNU","CRQA","LRQA","CRF","LRC","CRAV","CRA","CRI","ED"];
...
export default function App() {
  ...
  const [volarForm, setVolarForm] = useState(() => {
    const d = loadData();
    const currentExists = d.registros.find(r => r.ano === yearNow && r.mes === monthNow);
    if (currentExists) return { ano: yearNow, mes: monthNow, sectoresHoy: "" };
  ...
  function handleConfirmarPdf(reemplazar=false) {
    ...
    // If month exists and not confirmed replacement, show warning
    if (existe >= 0 && !reemplazar) {
      setConfirmarReemplazo(true);
      return;
    }
    const reg = { ano, mes, diasProgramados, sectoresProgramados,
      valorSector: data.valorSector,
      // Keep ejecutados and log if replacing, reset if new
  ...
  function handlePlanear() {
    ...
    if(existe>=0){
      if(!window._planConfirmed){
        showToast("Este mes ya existe. Edítalo desde el historial.","err");
        return;
      }
      window._planConfirmed=false;
    }
    if(existe>=0)registros[existe]=reg; else registros.push(reg);
    registros.sort((a,b)=>a.ano!==b.ano?a.ano-b.ano:a.mes-b.mes);
    setData(d=>({...d,registros}));
  ...
  // Racha: consecutive months meeting or exceeding programado
  const rachaActual = (() => {
    const sorted = [...regs].sort((a,b) => a.ano!==b.ano?a.ano-b.ano:a.mes-b.mes);
    let racha = 0;
    for (let i = sorted.length-1; i >= 0; i--) {
      const r = sorted[i];
      if (r.sectoresEjecutados >= r.sectoresProgramados) racha++;
      else break;
    }
    return racha;
  })();

  // Tendencia: compare last 3 months avg vs previous 3 months avg
  const tendencia = (() => {
    ...
    const pct = prev3.length > 0 ? Math.round((diff/avgPrev)*100) : 0;
    return { diff, pct, up: diff >= 0 };
  })();

  // Proyección anual - based on registered months, not current month
  const proyeccionAnual = (() => {
    const rsAnio = regs.filter(r=>r.ano===yearNow);
    if (rsAnio.length === 0) return null;
    const promMensual = rsAnio.reduce((s,r)=>s+calcIngreso(r),0)/rsAnio.length;
    ...
    const proyeccion = acumulado + (promMensual * mesesRestantes);
    return { proyeccion, promMensual, mesesRestantes, mesesRegistrados: rsAnio.length };
  })();
      ...
      <div style={{position:"sticky",top:0,zIndex:10}}>
        <div style={{background:"linear-gradient(135deg,#1a0a0a 0%,#7a1510 45%,#c0392b 100%)",padding:"22px 20px 18px",position:"relative",overflow:"hidden"}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
            ...
            <div style={{fontSize:10,color:"rgba(255,255,255,0.35)",letterSpacing:1,marginTop:4}}>v1.1.0</div>
          </div>
            ...
            const steps = [
              {
                ...
                desc:"Tu app para registrar y controlar tus sectores de chequeo como instructor TRI-R A320.",
            ...
            {regs.length>0&&(
              <>
                ...
                {mejorMes&&(()=>{
                  ...
                  const calcIngLocal = r => Math.max(r.sectoresEjecutados,r.sectoresProgramados)*(r.valorSector||data.valorSector);
                  const mejor = mpRegs.length ? mpRegs.reduce((a,b)=>calcIngLocal(a)>calcIngLocal(b)?a:b) : null;
                  const peor  = mpRegs.length ? mpRegs.reduce((a,b)=>calcIngLocal(a)<calcIngLocal(b)?a:b) : null;
                  return(
                      ...
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                        <div>
                          ...
                          {mejor ? <>
                            ...
                            <div style={{fontSize:13,color:"#7a8fa6"}}>{fmt(calcIngLocal(mejor))}</div>
                        ...
                        <div>
                          ...
                          {peor ? <>
                            ...
                            <div style={{fontSize:13,color:"#7a8fa6"}}>{fmt(calcIngLocal(peor))}</div>
                          </> : <div style={{fontSize:12,color:"#7a8fa6",marginTop:4}}>Sin datos</div>}
                        </div>
                      </div>
                    </div>
                                ...
                                <div style={{textAlign:"right"}}>
                                  ...
                                  {r.log&&r.log.length>0&&(
                                    <div style={{marginTop:8}}>
                                      <button onClick={()=>setLogsExpanded(prev=>({...prev,[`${r.ano}-${r.mes}`]:!prev[`${r.ano}-${r.mes}`]}))}
                                        ...
                                        <span style={{fontSize:11,transform:logsExpanded[`${r.ano}-${r.mes}`]?"rotate(180deg)":"rotate(0deg)",transition:"transform 0.2s",display:"inline-block"}}>▼</span>
                                      </button>
                                      {logsExpanded[`${r.ano}-${r.mes}`]&&(
                                          ...
                                          {[...r.log].reverse().map((entry,li)=>{
                                            ...
                                            return(
                                              <div key={li} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"5px 0",borderBottom:"1px solid #1a2533"}}>
                                                <div style={{fontSize:12}}><span style={{color:"#7a8fa6"}}>{entry.fecha}</span><span style={{marginLeft:8,color:"#3498db",fontWeight:600}}>+{entry.sectores} sector{entry.sectores>1?"es":""}</span></div>
              ...
              <div style={{marginBottom:20}}>
                ...
                {confirmarReemplazo ? (
                  <div style={{background:"rgba(231,76,60,0.1)",border:"1px solid rgba(231,76,60,0.4)",borderRadius:12,padding:"16px",marginBottom:12}}>
                    ...
                    <div style={{fontSize:13,color:"#7a8fa6",marginBottom:14}}>{pdfParsed&&MESES[pdfParsed.mes]} {pdfParsed&&pdfParsed.ano} ya tiene datos. Los sectores ejecutados se conservarán pero la programación se actualizará.</div>
        ...
        {[{id:"resumen",label:"Resumen",icon:"📊"},{id:"planear",label:"Programado",icon:"📋"},{id:"volar",label:"Ejecutado",icon:"✈️"}].map(tab=>(
          <button key={tab.id} onClick={()=>{
            if(tab.id==="volar"&&regs.length>0){
              ...
              if(currentExists){setVolarForm(f=>({...f,ano:yearNow,mes:monthNow}));}
              else{const last=regs[regs.length-1];setVolarForm(f=>({...f,ano:last.ano,mes:last.mes}));}
            }
