...
const CODIGOS_CHEQUEO = ["CRNU","CRQA","LRQA","CRF","LRC","CRA","CRI"];
...
function parsePdfText(fullText, fileName) {
  ...
  for (const code of CODIGOS_CHEQUEO) {
    const re = new RegExp(code + "\\s+\\d{4}", "g");
  ...
  // Each TRI-R chequeo day always has exactly 4 sectors (2 round trips).
  // This is consistent across all Avianca monthly schedules.
  const diasConChequeo = totalSectores > 0 ? Math.round(totalSectores / 4) : 0;
...
export default function App() {
  ...
  function handleActualizarValor() {
    ...
    // Only update the global value - historical months keep their own valorSector
    setData(d=>({...d, valorSector:v}));
    setNuevoValor("");
  ...
  function handleExportar() {
    ...
    const promMensual = data.registros.length>0 ? fmt(total/data.registros.length) : fmt(0);
  ...
  const totalIngresos = regs.reduce((s,r)=>s+r.ingresoActual,0);
  const anios = [...new Set(regs.map(r=>r.ano))].sort();
  const mejorMes = regs.length?regs.reduce((a,b)=>a.ingresoActual>b.ingresoActual?a:b):null;
  const peorMes = regs.length?regs.reduce((a,b)=>a.ingresoActual<b.ingresoActual?a:b):null;
  ...
  return (
    <div style={{minHeight:"100vh",background:"#0f1923",color:"#e8edf2",fontFamily:"'Inter','Segoe UI',sans-serif",paddingBottom:90}}>
      ...
      <div style={{maxWidth:520,margin:"0 auto",padding:"0 16px"}} ref={resumenRef}>
          ...
          <div>
                  ...
                  <div style={{marginTop:12,borderTop:"1px solid #2d3f52",paddingTop:12}}>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                      <div>
                        ...
                        <div style={{fontSize:13,fontWeight:600,color:"#7a8fa6"}}>{mesAnterior.sectoresEjecutados} sect. · {fmt(mesAnterior.ingresoActual)}</div>
                      </div>
                      <div>
                        {(()=>{
                          const ds=mesActual.sectoresEjecutados-mesAnterior.sectoresEjecutados;
                          const dc=mesActual.ingresoActual-mesAnterior.ingresoActual;
              ...
              <>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                  {[
                    ...
                    {label:"Prom. Mensual",value:fmt(regs.length>0?totalIngresos/regs.length:0),color:"#f39c12"},
                ...
                {mejorMes&&(()=>{
                  ...
                  const mejor = mpRegs.length ? mpRegs.reduce((a,b)=>a.ingresoActual>b.ingresoActual?a:b) : null;
                  const peor  = mpRegs.length ? mpRegs.reduce((a,b)=>a.ingresoActual<b.ingresoActual?a:b) : null;
                  return(
                      ...
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                        <div>
                          ...
                          {mejor ? <>
                            ...
                            <div style={{fontSize:13,color:"#7a8fa6"}}>{fmt(mejor.ingresoActual)}</div>
                        ...
                        <div>
                          ...
                          {peor ? <>
                            ...
                            <div style={{fontSize:13,color:"#7a8fa6"}}>{fmt(peor.ingresoActual)}</div>
                          </> : <div style={{fontSize:12,color:"#7a8fa6",marginTop:4}}>Sin datos</div>}
                        </div>
                      </div>
                    </div>
                ...
                {(()=>{
                  ...
                  const ing = rs.reduce((s,r)=>s+r.ingresoActual,0);
                  const efic = prog>0?Math.round((ejec/prog)*100):0;
            ...
            <div style={{marginTop:20}}>
              ...
              {anios.slice().reverse().map(anio=>{
                ...
                const ingAnio=regsAnio.reduce((s,r)=>s+r.ingresoActual,0);
                return(
                            ...
                            <div key={i} style={{background:"#1a2533",padding:"12px 16px",borderBottom:i<regsAnio.length-1?"1px solid #1e2d3d":"none"}}>
                              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                                <div>
                                  ...
                                  {r.valorSector&&r.valorSector!==data.valorSector&&(
                                    <div style={{fontSize:11,color:"#5b9bd5",marginTop:2}}>Valor: {fmt(r.valorSector)}/sector</div>
                                  )}
                                </div>
                                <div style={{textAlign:"right"}}>
                                  <div style={{fontSize:16,fontWeight:700,color:"#2ecc71"}}>{fmt(r.ingresoActual)}</div>
                                  <div style={{
        ...
        {view==="exportar"&&(
          <div style={{marginTop:20}}>
            <div style={{fontSize:18,fontWeight:700,marginBottom:6}}>Exportar resumen</div>
            <div style={{fontSize:13,color:"#7a8fa6",marginBottom:20}}>Genera un reporte PDF con todo tu historial.</div>
            <div style={{background:"#1a2533",borderRadius:14,padding:"16px 18px",marginBottom:20}}>
              <div style={{fontSize:12,color:"#7a8fa6",marginBottom:8}}>Incluye</div>
              {["Totales históricos","Ingresos por mes","Sectores programados vs ejecutados","Eficiencia y comparativos"].map((item,i)=>(
                <div key={i} style={{fontSize:13,color:"#e8edf2",padding:"6px 0",borderBottom:"1px solid #1e2d3d",display:"flex",gap:8,alignItems:"center"}}>
            ...
            <div style={{background:"rgba(243,156,18,0.1)",border:"1px solid rgba(243,156,18,0.3)",borderRadius:10,padding:"12px 16px",marginBottom:20,fontSize:13,color:"#f39c12"}}>
              📱 En celular: al abrirse el reporte, toca el botón compartir y elige "Guardar como PDF" o "Imprimir".
            </div>
            <button onClick={handleExportar} style={{width:"100%",background:"#2ecc71",border:"none",color:"#0f1923",padding:"14px",borderRadius:12,fontSize:16,fontWeight:700,cursor:"pointer",marginBottom:12}}>📄 Generar reporte</button>
            <button onClick={()=>setView("resumen")} style={{width:"100%",background:"transparent",border:"1px solid #2d3f52",color:"#7a8fa6",padding:"12px",borderRadius:12,fontSize:15,cursor:"pointer"}}>Cancelar</button>
          </div>
        )}
        ...
        {view==="config"&&(
          <div style={{marginTop:20}}>
            ...
            <div style={{background:"rgba(243,156,18,0.1)",border:"1px solid rgba(243,156,18,0.3)",borderRadius:10,padding:"12px 16px",marginBottom:20,fontSize:13,color:"#f39c12"}}>
              ✓ Actualizar solo afecta meses nuevos. Los meses ya registrados conservan su valor original.
            </div>
      ...
      <div style={{position:"fixed",bottom:0,left:0,right:0,background:"#111d2b",borderTop:"1px solid #1e2d3d",display:"flex",justifyContent:"space-around",padding:"10px 0 16px"}}>
        {[{id:"resumen",label:"Resumen",icon:"📊"},{id:"planear",label:"Programado",icon:"📋"},{id:"volar",label:"Ejecutado",icon:"✈️"},{id:"exportar",label:"Exportar",icon:"📄"}].map(tab=>(
